...
| 功能 | 说明 | 实现要点 |
|------|------|----------|
| **文本描述** | 支持输入一段文字（**500 字以内**）描述该 box 所包含的模型结构 | 在标注表单中增加「描述」输入框（如 `<textarea>`），限制最大长度 500 字；保存时写入该 target 或该标注的字段。若以 target 为单位，则每个 target 可单独有描述；若以标注为单位，则一条标注一个描述。后端/数据库需新增字段（如 `description TEXT` 或 annotations 表增加 `description`），并在 API 与迁移中支持。 |
